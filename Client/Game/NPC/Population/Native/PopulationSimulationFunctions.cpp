#include "PopulationSimulationFunctions.h"

#include <cstring>

namespace fable::game::npc::population::native
{
    bool PopulationSimulationFunctions::ResolveProcessAlbion(
        HMODULE gameModule,
        std::uint8_t*& address) noexcept
    {
        return Resolve(
            gameModule,
            ProcessAlbionAddressRva,
            ProcessAlbionExceptionHandlerRva,
            address);
    }

    bool PopulationSimulationFunctions::ResolveHighDetail(
        HMODULE gameModule,
        std::uint8_t*& address) noexcept
    {
        return Resolve(
            gameModule,
            HighDetailAddressRva,
            HighDetailExceptionHandlerRva,
            address);
    }

    bool PopulationSimulationFunctions::Resolve(
        HMODULE gameModule,
