...
namespace fable::game::hero_pawn::abilities::hooks
{
    ...
    bool HeroWillAbilityHook::Detour::Install(
        ...
        std::size_t bytes) noexcept
    {
        ...
        if (targetAddress == nullptr || replacement == nullptr || bytes < 5)
        {
            return false;
        }
    ...
    bool HeroWillAbilityHook::Detour::Reset() noexcept
    {
        return patch.Shutdown();
    }
        ...
        const core::Diagnostics& diagnostics)
    {
        ...
        {
            diagnostics.Event(
                "HeroWillAbilityHookRejected",
                "native definitions failed validation");
            return false;
        }

        // Publish the owner before a partial install so a retained detour can
        // still chain through its original if rollback encounters a competing
        // hook.
        service_ = &service;
        gameModule_ = gameModule;
        diagnostics_ = diagnostics;
        active_ = this;
        if (!use_.Install(use, reinterpret_cast<void*>(&InterceptUse)) ||
            !toggle_.Install(
                toggle, reinterpret_cast<void*>(&InterceptToggle)) ||
            !cancel_.Install(
                cancel, reinterpret_cast<void*>(&InterceptCancel)) ||
            !eligibility_.Install(
                eligibility,
                reinterpret_cast<void*>(&InterceptEligibility)) ||
            !turncoatState_.Install(
                turncoatState,
                    ...
                    TurncoatStateDisplacedBytes))
        {
            ...
            std::snprintf(
                ...
                use,
                use_.IsInstalled() ? "true" : "false",
                toggle,
                toggle_.IsInstalled() ? "true" : "false",
                cancel,
                cancel_.IsInstalled() ? "true" : "false",
                eligibility,
                eligibility_.IsInstalled() ? "true" : "false",
                turncoatState,
                turncoatState_.IsInstalled() ? "true" : "false");
            ...
            if (toggle_.IsInstalled())
            {
                originalToggle_ = reinterpret_cast<
                    native::HeroWillAbilityFunctions::CommandPointer>(
                        toggle_.Original());
            }
            if (cancel_.IsInstalled())
            {
                originalCancel_ = reinterpret_cast<
                    native::HeroWillAbilityFunctions::CommandPointer>(
                        cancel_.Original());
            }
            if (eligibility_.IsInstalled())
            {
                originalEligibility_ = reinterpret_cast<
                    native::HeroWillAbilityFunctions::CommandPointer>(
                        eligibility_.Original());
            }
            if (turncoatState_.IsInstalled())
            {
                originalTurncoatState_ = reinterpret_cast<
                    native::HeroWillAbilityFunctions::TurncoatStatePointer>(
                        turncoatState_.Original());
            }
            bool allReset = true;
            allReset = use_.Reset() && allReset;
            allReset = toggle_.Reset() && allReset;
            allReset = cancel_.Reset() && allReset;
            allReset = eligibility_.Reset() && allReset;
            allReset = turncoatState_.Reset() && allReset;
            if (!allReset)
            {
                diagnostics.Log(
                    "Hook: Hero Will rollback deferred because a target is owned by another hook.");
            }
            else
            {
                active_ = nullptr;
        ...
        originalUse_ = reinterpret_cast<
            native::HeroWillAbilityFunctions::CommandPointer>(use_.Original());
        originalToggle_ = reinterpret_cast<
            native::HeroWillAbilityFunctions::CommandPointer>(
                toggle_.Original());
        originalCancel_ = reinterpret_cast<
    ...
    void HeroWillAbilityHook::Shutdown() noexcept
    {
        ...
        allRestored = turncoatState_.Reset() && allRestored;
        allRestored = eligibility_.Reset() && allRestored;
        allRestored = cancel_.Reset() && allRestored;
        allRestored = toggle_.Reset() && allRestored;
        allRestored = use_.Reset() && allRestored;
        if (!allRestored)
        {
            diagnostics_.Log(
                "Hook: Hero Will shutdown deferred because a target is owned by another hook.");
            return;
        }
        if (active_ == this)
        {
    ...
    bool HeroWillAbilityHook::IsInstalled() const noexcept
    {
        return active_ == this && service_ != nullptr && gameModule_ != nullptr &&
            ...
            originalCancel_ != nullptr && use_.IsInstalled() &&
            toggle_.IsInstalled() && cancel_.IsInstalled() &&
            originalEligibility_ != nullptr && eligibility_.IsInstalled() &&
            originalTurncoatState_ != nullptr && turncoatState_.IsInstalled();
    }
